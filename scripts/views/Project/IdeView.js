define([
  ...
  var IdeView = Backbone.Epoxy.View.extend({
    ...
    events: {
      ...
      'click .save:not(.autosave)': function (e) {
        ...
        if (!this.model.get('saved')) {
          if (Pi.user.isGuest()) window.location.hash = '#log-in'
          else this.model.saveSketch(true)
        }
      },
      'click .autosave': function (e) {
        e.stopPropagation()
        if (Pi.user.isGuest()) window.location.hash = '#log-in'
        else this.model.set('autoSave', !this.model.get('autoSave')) // toggle autosave
      },
      ...
      'click .rename_tab': function (e) {
        ...
        if (!activeTab) activeTab = this.model.getMainTab()
        activeTab.set({
          active: true,
          editMode: true
        })
     ...
     */
    openState: function () {
      if (this.model.get('open')) this.$el.show()
      else this.$el.hide()
    },
    /**
     * Active state.
    ...
    getLongTitle: function () {
      return (
        this.model.get('name') + ' - ' + Pi.shortName + ' (' + Pi.version + ')'
      )
    },
    /**
