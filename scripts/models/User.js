define([
  ...
  var User = Backbone.RelationalModel.extend({
     ...
     */
    safeAttributes: ['username', 'lastvisit_at'],
    /**
     * User init.
     */
    initialize: function () {
      ...
      if (!this.isGuest()) this.update(Pi.bootstrap, false)
    },
    /**
    ...
    desktopBootstrap: function (data) {
      // console.log('desktopBootstrap', this.isGuest(), this.bootstrapped, data)
      ...
      } else if (!this.projectsBootstrapped) {
        var p = data && data.user ? data.user.projects || [] : []
        console.log(p)
        this.openProjects(p)
    ...
    update: function (data, fetchProjects) {
      // console.log('UPDATE', data, fetchProjects)
     ...
     * @param {json} projects
     */
    openProjects: function (projects) {
      _.each(
        projects,
        function (project) {
          if (project.open) {
            ProjectController.open(project.id, null, null, true, false)
          }
        },
        this
      )
     ...
     */
    getAvatar: function () {
      if (this.get('avatar')) return this.get('avatar')
      //Pi.user.getPublicDir() + "/avatar.jpg";
      else return Pi.imgPath + '/' + Pi.defaultAvatarFileName
    },
    /**
