define([
  ...
  var Project = Backbone.RelationalModel.extend({
     ...
     */
    getUid: function () {
      if (!this.uid) this.uid = Pi.js.generateUid()
      return this.uid
    },
    /**
    ...
    stopSketch: function (options) {
      ...
      if (this.outputView)
        this.outputView.iframeWindow().postMessage(
      ...
      if (!_.isEmpty(this.outputView)) {
        if (options && options.hide) this.outputView.hide()
      }
      this.set({
        running: false,
        isPaused: true
    ...
    saveOpenState: function () {
      if (!Pi.user.isGuest() && !this.isNew()) {
        var state = {}
        var left = parseInt(this.get('left'))
        var top = parseInt(this.get('left'))

        state['open'] = this.get('open') ? 1 : 0
        ...
        state[Pi.csrfTokenName] = Pi.csrfToken
    ...
    saveTabs: function () {
      ...
      this.get('tabs').each(function (tab) {
        ...
        $.when(tab.smartSave())
          .done(function () {
            saveCounter++
            if (tabsLength === saveCounter) saved.resolve()
          })
          .fail(function () {
            saved.reject()
          })
    ...
    askForSave: function () {
      ...
      if (!Pi.user.isGuest()) {
        ...
        Pi.confirmation({
          ...
          message:
            "If you want to save your work, please <strong><a href='#log-in'>log in</a></strong>.<br>" +
    ...
    getOutputLeft: function () {
      ...
      var bestPosition =
        this.ideView.$el.width() + parseInt(this.ideView.$el.css('left')) + 50
